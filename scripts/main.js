...
(function () {
	...
	function offlineLocalLinks() {
		var localUrls = $('a').filter(function (i) {
			return(

				// Cache all https local resources
				i.hostname === location.hostname && i.protocol === 'https'
			);
		}).map(function (i) {
			return i.toString();
		});

		return storeStaticResources(localUrls).then(function (result) {
			if (result.success) {
	...
	if ('serviceWorker' in navigator && location.protocol === 'https:') {
		...
		if (navigator.serviceWorker.controller) {
			...
			$('.hero-offline a').on('click', function () {
				...
				(window.offlineLinks ? storeStaticResources(window.offlineLinks) : offlineLocalLinks()).then(function (data) {
